...
const defaultConfig = {
  rules: [{ test: /\.svg$/, value: "sprite.svg" }],
  symbolFactory,
  spriteFactory
};
  ...
  addSymbol({ path, content, id }) {
    ...
    return SpriteSymbol.create(options).then(newSymbol => {
      ...
      const allExceptCurrent = symbols
        .filter(
          s => s.request.fileEquals(request) && !s.request.queryEquals(request)
        )
      ...
      return Promise.map(allExceptCurrent, ({ symbol, index }) => {
        ...
        return SpriteSymbol.create(opts).then(
          created => (symbols[index] = created)
        );
      }).then(() => newSymbol);
    });
  }
